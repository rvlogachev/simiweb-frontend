...
use app\commands\command\SendEmailCommand;
use app\commands\command\SendSocketCommand;
...
class MessagerController extends Controller
{
    ...
    public function actionSend($type, $title, $body, $email = 'info@ebitda.ru', $order_id = 0, $phone = '',$client='backend')
    {
    ...
    public function actionRun($exchange = 'direct', $queue = 'messages', $consumer_tag = '')
    {
        ...
        $conn = new AMQPConnection('172.30.0.2', '5672', 'guest', 'guest', '/');
        ...
        $process_message = function ($msg) {
            ...
            if (isset($body['type']) && isset($body['title']) && isset($body['body']) && !is_array($body['body'])) {

                $key = md5($body['type'] . $body['title'] . $body['body'] . time());
                ...
                switch ($body['type']) {
                    case MessagePlan::TYPE_EMAL:
                        $command = "php " . Yii::$app->basePath . "/yii  messager/ressive-email '{$key}' '{$body['title']}' '{$body['body']}' '{$email}' ";
                        break;
                    case MessagePlan::TYPE_SLACK:
                        $command = "php " . Yii::$app->basePath . "/yii  messager/ressive-slack '{$key}' '{$body['body']}' '{$body['chenel']}'";
                        break;
                    case MessagePlan::TYPE_SMS:
                        $command = "php " . Yii::$app->basePath . "/yii  messager/ressive-sms '{$key}.' '{$body['phone']}' '{$body['body']}'";
                        break;
                    case MessagePlan::TYPE_SOCKET:
                        $command = "php " . Yii::$app->basePath . "/yii  messager/ressive-socket '{$key}.' '{$body['body']}' '{$body['client']}'";
                        break;
                    case MessagePlan::TYPE_ADD_LEAD:

                        $command = "php " . Yii::$app->basePath . "/yii  messager/new-lead '{$key}.' '{$body['body']}' '{$body['cookie']}' '{$body['phone']} ' '{$body['url']} ' '{$body['miniland_id']} ' '{$body['title']} '";

                        break;
                    case MessagePlan::TYPE_ADD_LEAD_AMO:
                        $command = "php " . Yii::$app->basePath . "/yii  messager/add-lead-amo '{$key}.' '{$body['body']}' '{$body['cookie']} '";
                        break;
                }
                ...
                if (isset($body['data_send']) && $command && $body['data_send'] == date("Y-m-d")) {
                    ...
                    $this->stdout("Отправлено сообщение  pid = " . $processId . " type: " . $body['type'] . " текст: " . $body['body'] . " key: " . $key . "\r\n", Console::FG_GREEN);
                } else {

                    $this->stdout("Подготовлено сообщение  pid = " . $processId . " type: " . $body['type'] . " текст: " . $body['body'] . " key: " . $key . "\r\n", Console::FG_GREEN);
                }
...
//            // acknowledge message
    ...
    public function actionRessiveSocket($key, $body, $client)
    {



        Yii::error("actionRessiveSocket: {$key}, {$body}, {$client} ",'chat');
